...
from src.models.models import Query as QueryModel, QueryPaper, QueryResponse, QueryHistoryResponse, Paper, QueryAnswer, Citation, PaperChunk
...
async def analytics_popular(
    db: AsyncSession = Depends(get_db),
    n_clusters: int = 10,  # configurable via query params
):
    """
    Return most semantically popular topics (clusters) asked by users.
    Each topic is represented by a cluster center/exemplar query, sorted by frequency/popularity.
    """
    # Fetch all queries
    result = await db.execute(select(QueryModel))
    queries = result.scalars().all()
    if not queries:
        return []
    ...
    # Step 3: For each cluster, find the representative query (closest to centroid)
    ...
    for idx, member_indices in enumerate(clusters):
        ...
        center = centers[idx].reshape(1, -1)
        sims = cosine_similarity(cluster_embs, center).flatten()
        best_idx = member_indices[int(np.argmax(sims))]
        cluster_texts = [texts[i] for i in member_indices]
        cluster_infos.append({
            "topic_exemplar": texts[best_idx],
            "count": len(member_indices),
            "example_queries": cluster_texts[:5],
        })
