...
# Force SSL mode to require for cloud databases
if 'neon.tech' in raw_db_url or 'amazonaws.com' in raw_db_url:
    if 'sslmode=' not in raw_db_url:
        raw_db_url += '?sslmode=require'
    elif '?' in raw_db_url and 'sslmode=' not in raw_db_url:
        raw_db_url += '&sslmode=require'

# Parse the URL and handle connection parameters
...
if 'channel_binding' in query_params:
    ...
    logger.info("Removed channel_binding parameter from connection string")
...
# Create async engine with adjusted configuration for asyncpg 0.30.0
engine = create_async_engine(
    DATABASE_URL,
    connect_args=connect_args,
    echo=False,
    future=True,
    isolation_level="READ COMMITTED",  # Explicitly set isolation level
    pool_pre_ping=True,  # Enable connection health checks
    pool_size=20,        # Set pool size
